...
pub fn calculate_hma(input: &HmaInput) -> Result<HmaOutput, Box<dyn Error>> {
            ...
            if in_idx_full < len {
                ...
                while in_idx<len||in_idx_full<len {
                    if in_idx<len {
                        ...
                        period_sum_half+=new_val*(half as f64);
    ...
    let mut wma_sqrt=vec![f64::NAN;len];
    {
        ...
        if in_idx_sqrt<len {
            ...
            if val.is_finite() {
                period_sub_sqrt+=val;
                period_sum_sqrt+=val*(sqrtp as f64);
            }
            let trailing_val_sqrt=diff[0];
            let mut trailing_idx_sqrt=1;
            wma_sqrt[lookback_sqrt]=if trailing_val_sqrt.is_finite() {
                period_sum_sqrt/denom_sqrt
            } else {f64::NAN};
            period_sum_sqrt-=period_sub_sqrt;
            let mut trailing_value_sqrt=trailing_val_sqrt;
            while in_idx_sqrt<len {
                ...
                if new_val.is_finite() {
                    period_sum_sqrt+=new_val*(sqrtp as f64);
                }
                trailing_value_sqrt=diff[trailing_idx_sqrt];
                trailing_idx_sqrt+=1;
                wma_sqrt[in_idx_sqrt-1]=if period_sub_sqrt!=0.0 {period_sum_sqrt/denom_sqrt}else{f64::NAN};
                period_sum_sqrt-=period_sub_sqrt;
            }
        }
    }
...
mod tests {
    ...
    fn test_hma_accuracy() {
        ...
        for (i,&val) in last_five.iter().enumerate() {
            ...
            assert!((val-exp).abs()<1e-3,"HMA mismatch at {}: expected {}, got {}",i,exp,val);
        }
    }
}
